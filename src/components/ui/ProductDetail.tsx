    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          {/* Unobstructed Cinematic Visual Showcase */}
          <div className="relative w-full lg:w-3/5 h-[50vh] lg:h-full overflow-hidden bg-black flex items-center justify-center">
            ...
            <div className="absolute bottom-20 left-20 space-y-4 z-10">
              <div className="flex gap-2">
                {[...Array(5)].map((_, i) => (
                  <Star key={i} size={12} className={i < Math.floor(product.rating) ? "text-primary fill-primary" : "text-white/20"} />
                ))}
              </div>
              <p className="text-[10px] uppercase tracking-[0.6em] text-white/40 font-bold">Authentic Commission</p>
            </div>
          </div>

          {/* Luxury Editorial Information */}
          <div className="w-full lg:w-2/5 p-12 lg:p-24 flex flex-col justify-center space-y-12 overflow-y-auto bg-black border-l border-white/5 relative z-20">
            ...
            <div className="space-y-8">
              <div className="flex items-center gap-4">
                <span className="text-[10px] uppercase tracking-[0.6em] text-white/30 font-bold whitespace-nowrap">The Elements</span>
            ...
            <div className="pt-12 space-y-10">
              <div className="flex items-end justify-between">
                <div className="space-y-2">
                  <span className="text-[10px] uppercase tracking-[0.4em] text-white/20 font-bold">Total Investment</span>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <Button 
                  size="lg" 
                  className="flex-1 h-20 rounded-none bg-white text-black text-[10px] font-bold uppercase tracking-[0.5em] hover:scale-105 transition-transform"
                  onClick={() => {
                    onAddToCart(product, qty);
                    onClose();
                  }}
                >
                  <CreditCard size={14} className="mr-3" /> Secure Purchase
                </Button>
                <Button 
                  size="lg" 
                  variant="outline"
